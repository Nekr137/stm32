...
int main()
{
	...
	USART_config();
	USART_SendSTR("Nekrasov Denis");
	
	while(1) 
	{
		while((USART1->ISR & USART_ISR_RXNE) == USART_ISR_RXNE)
		{
			receive = USART1->RDR; /* Receive data, clear flag */
			USART_SendSTR(&receive); //send_to_uart 
		}

		if(myTick>1500)
		{
			myTick = 0;
			USART_SendCharacter(0x08);//backspace
		}
	}
...
void USART_config(void)
{
	...
	//0x341 ????? ?? 9 ??? - 920??? - ????
	USART1 -> BRR = 0x341; //0x341;//0x1A0B;//48000000/12;//480000/96;//0x341;//0x1601;//0x9C4;//0x34D;//480000/96;//1388;//0x681; //0x681;//0x9C4;// 0x681; 0x341h 0x64
