...
//uint8_t http_header[] = {"HTTP/1.1 200 OKrnServer: nginxrnContent-Type: text/htmlrnConnection: keep-alivernrn"};

uint8_t data[] = {
	
};

uint8_t ip_home[] = "192.168.1.183\r\n";
//uint8_t ConnectBrowser[] = "HTTP/1.1 200 OK Server: nginx Content-Type: text/html Connection: keep-alive\r\n\r\n";
//uint8_t ConnectBrowser[] = "HTTP/1.1 200 OK Server: nginx Content-Type: text/html Connection: reconnect\r\n\r\n";
...
int main()
{
	...
	//USART_SendSTR("AT+CIPSTART=0,\"UDP\",\"192.168.1.183\",80\r\n",0,4);//??????????????????
//	while(!StringExist("CONNECTED"));
//	clear_message4();
	...
	{	
		...
		int size = sizeof(d1);+sizeof(d2)+sizeof(d3)+sizeof(d4)+sizeof(d5)+sizeof(d6)+sizeof(d7);
		USART_SendSTR("AT+CIPSEND=0,",0,4);		USART_SendINT(size+24,4);		USART_SendSTR("\r\n",0,4);	
		SETLEDblue();
		Pause(3000);
		USART_SendChar(d1,4); 
//		USART_SendChar(d2,4); 
...
//		Pause(1000);
//		USART_SendChar(d7,4);
		USART_SendSTR("AT+CIPCLOSE=0\r\n",0,4);
		SETLEDgreen();
//		USART_SendChar(d2,4);//Pause(2000);
//				USART_SendChar(d3,4);//Pause(2000);
//		USART_SendChar(d4,4);//Pause(2000);
		Pause(30000);
		

//		while(!StringExist("OK"))
//		{
//			
//		}
		//char command[] = "";
		...
		cnt = 0;
		clear_message4();
		//USART_SendChar(command,4);	//57
		//USART_SendSTR("HTTP/1.0 200 OK\r\nServer: nginx\r\nContent-Type: text/html\r\nConnection: close\r\n\r\n",0,4);
		//USART_SendChar("",4);//14
		//USART_SendChar("+++",4);
...
void ESP8266initPoint(void)
{
	...
	//USART_SendSTR("AT+CWJAP=\"HUAWEI P10\",\"yanayana\"\r\n",0,4);
	while(!StringExist("OK"));
	clear_message4();
	// by best is will be remain 192.168.1.246
	//USART_SendSTR("AT+CIPSTA=\"192.168.4.4\"\r\n",0,4);
	//while(!StringExist("OK"));
	clear_message4();
	USART_SendSTR("AT+CIPMUX=1\r\n",0,4);	
	while(!StringExist("OK")); 
	clear_message4();	Pause(500);
	USART_SendSTR("AT+CIPSERVER=1,80\r\n",0,4);
	while(!StringExist("OK")); 
	clear_message4();
